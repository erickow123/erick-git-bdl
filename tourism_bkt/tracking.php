<?php
    ...
    */
    ...
    //AWAL
	$querysearch="SELECT id, geom, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT(".$lng1." ".$lat1.")',-1), geom) as jarak FROM angkot where st_distance_sphere(ST_GeomFromText('POINT(".$lng1." ".$lat1.")',-1), geom) <= ".$rad.""; 
    ...
    //TUJUAN
    $querysearch="SELECT id, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT(".$lng2." ".$lat2.")',-1), geom) as jarak FROM angkot where st_distance_sphere(ST_GeomFromText('POINT(".$lng2." ".$lat2.")',-1), geom) <= ".$rad.""; 
    $hasil=mysqli_query($conn, $querysearch);
    ...
    for ($i=0; $i < $pjg; $i++) { 
        if ($or == "") {
            $or = $or." id= '".$angkot_tujuan[$i]."'";             
        }else{
            $or = $or." OR id= '".$angkot_tujuan[$i]."'";             
        }
    }
    //echo "<br> $or <br>";
    ...
    for ($x=0; $x < count($angkot_awal); $x++) { 
        for ($i=0; $i < count($angkot_tujuan); $i++) { 
            ...
            if ($z == count($angkot_tujuan)) {
                ...
                $querysearch="SELECT id, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere('".$geom_awal[$x]."', geom) as jarak FROM angkot where ".$or."order by jarak LIMIT 1"; 
                $hasil=mysqli_query($conn, $querysearch);
